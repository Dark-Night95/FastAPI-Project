...
class Patient(BaseModel):
    ...
    age: Annotated[int, Field(..., description="age of patient", lt=125)]
    gender: Annotated[
    ...
    height: Annotated[
        float,
        Field(..., description="height of patient in mtrs", gt=0),
    ]
    weight: Annotated[
        float,
        Field(..., description="weight of patient in kgs", gt=0),
    ]
...
class PatientUpdate(BaseModel):
    ...
    age: Annotated[Optional[int], Field(default=None, lt=125)]
    gender: Annotated[
...
@app.get("/")
def hello():
    return {"message": "Patients Record"}


@app.get("/info")
def info():
    return {"purpose": "Patients Data"}
...
@app.get("/sort")
def sort_patient(
    sort_by: str = Query(..., description="Search patient by height,weight or bmi"),
    ...
    if sort_by not in valid_fields:
        raise HTTPException(
            status_code=400,
            detail=f"Invalid select field from {valid_fields}",
        )
